...
![Интерактивный просмотр дерева](docs/screenshots/tree.png)
*gedcom-tree — интерактивный просмотр*

![Аналитический дашборд](docs/screenshots/report.png)
*gedcom-report — аналитический дашборд*

<sub>Оба сгенерированы из вымышленного примера `examples/demo.ged`. Интерфейс
локализован — кириллические деревья отображаются по-русски, латинские —
по-английски (или задайте явно: `--lang ru|en`).</sub>

## Четыре навыка

| Навык | Что делает |
|---|---|
| **gedcom-reader** | Читает `.ged` и отвечает на вопросы о людях, датах и родстве. Также строит и редактирует деревья — добавляет людей, задаёт факты, связывает супругов и детей — с автоматическими бэкапами, и общий структурный **`audit`** (битые ссылки, циклы, пропущенные даты). **Начните отсюда.** |
| **gedcom-report** | Аналитический **дашборд** из 13 разделов: подсчёты, графики, облако имён, тепловая карта дней рождения, таймлайн, сводка **связанных лиц (ASSO/RELA)**, **покрытие источниками** и проверка качества данных — одним **полностью автономным** HTML-файлом (Chart.js встроен в файл, без CDN). |
| **gedcom-tree** | **Интерактивный HTML-просмотр дерева** с центром на человеке: предки сверху, потомки снизу, клик для перецентровки, панорама/зум, поиск по имени — плюс **панель ⓘ** на каждой карточке (заметки, источники с чипами QUAY, ссылки на документы/сканы, родня и **связанные лица**). |
| **genealogy-research** | Планирование исследования по **стандарту генеалогического доказательства (GPS)** (статусы доказанности, атомарные утверждения, протокол ограниченного запуска), ведение хранилища Obsidian и **построение дерева с нуля** через короткое интервью. |
...
одного `gedcom-reader.zip`.) Включённые здесь навыки также доступны в надстройках
Claude для Excel / Word / PowerPoint / Outlook.
...
Вышла новая версия? Обновите ту копию, которую уже установили:

- **Claude Desktop / claude.ai** — скачайте новые `.zip` из
  [`download-skills/`](download-skills/) и загрузите их заново через
  **Customize → Skills → «+» → Upload a skill**. Если Claude продолжает
  показывать старую версию, сначала удалите навык из списка Skills, затем
  загрузите снова.

- **Claude Code · Codex · opencode · другие CLI-агенты** — обновите этот
  репозиторий, затем повторно запустите ту же команду установки из корня проекта
  и перезапустите агента:

  ```bash
  git pull                      # или заново скачайте репозиторий
  ./install.sh opencode         # та же цель, что и раньше (+ project-dir, если был)
  ```

  Повторный запуск **перезаписывает** установленные папки `skills/*` на месте
  (это копии, так что безопасно) — вы всегда получаете свежие скрипты.

- **opencode, важная оговорка** — чтобы не затереть ваши правки, установщик
  **не перезаписывает** существующие `opencode.json`, `package.json` и
  `genealogist.md`, если вы его меняли; нативные инструменты `gedcom_*`
  обновляются всегда. Если в новой версии изменились агент, конфиг или
  зависимости плагина, установщик выведет `= … left untouched` — сравните свой
  файл с соответствующим в [`opencode-extras/`](opencode-extras/) и внесите
  изменения вручную (или переименуйте свой файл и запустите установщик снова,
  чтобы получить свежую копию).
...
# Построить аналитический дашборд  ->  examples/demo.report.html  (работает без интернета)
PYTHONIOENCODING=utf-8 python3 skills/gedcom-report/scripts/report.py examples/demo.ged
# ...безопасно для публикации (живущие исключены, прерывается при любой утечке):
PYTHONIOENCODING=utf-8 python3 skills/gedcom-report/scripts/report.py examples/demo.ged --share

# Проверить структуру дерева (битые ссылки, циклы, пропущенные даты)
...
→ братья и сёстры), примет ответ *«не знаю»* и приблизительные годы, а сведения
по памяти зафиксирует с указанием, кто и когда рассказал, и явной степенью
уверенности — не выдавая их за документально подтверждённые и не додумывая
неназванные родственные связи.
...
работают **локально** и не обращаются к сети — отчёт и просмотр дерева
**полностью автономны** (Chart.js встроен в файл, без CDN). Браузерный MCP —
опционален и включается только когда вы сами запускаете исследование через браузер.

Когда нужно **поделиться** деревом или дашбордом, у `gedcom-report` и `gedcom-tree`
есть два взаимоисключающих флага:

- `--private` — *именной семейный вид*: имена и граф остаются, но точные даты
  рождения, места и семейные события **возможно живущих** людей вырезаются.
  Баннер прямо сообщает, что это **не анонимно**.
- `--share` — *исторический экспорт с отказом «по умолчанию»*: возможно живущие и
  люди с неизвестным статусом исключаются полностью, имя исходного файла
  скрывается, а генерация **прерывается**, если аудит полезной нагрузки находит
  утечку защищённых данных. Используйте это для публикации.

`.gitignore` этого репозитория держит все `*.ged` вне контроля версий, кроме
вымышленных примеров — держите свои семейные данные локально.
