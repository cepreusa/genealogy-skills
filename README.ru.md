...
| **gedcom-tree** | **Интерактивный HTML-просмотр дерева** с центром на человеке: предки сверху, потомки снизу, клик для перецентровки, панорама/зум, поиск по имени — плюс **панель ⓘ** на каждой карточке (заметки, источники, ссылки на документы/сканы, родня). |
