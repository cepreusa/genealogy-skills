...
I18N = {
    "ru": {
        ...
        # scan-manifest integrity
        "sec_scans": "Целостность документов",
        "sec_scans_hint": "сверка сканов с манифестом",
        "sc_referenced": "Ссылок на файлы",
        "sc_verified": "Проверено",
        "sc_missing": "Отсутствует",
        "sc_mismatch": "Не совпадает",
        "sc_unmanifested": "Нет в манифесте",
        "sc_unchecked": "Хэш не сверялся",
        "sc_invalid": "Недопустимый путь",
        "sc_unsupported": "Неподдерживаемый алгоритм",
        "sc_external": "Внешние ссылки",
        "sc_problems": "Проблемные файлы",
        "sc_paths_hidden": "Пути к файлам скрыты приватным режимом.",
        "col_state": "Статус", "col_path": "Путь",
        # footer
    ...
    "en": {
        ...
        # scan-manifest integrity
        "sec_scans": "Document integrity",
        "sec_scans_hint": "scans checked against the manifest",
        "sc_referenced": "Referenced files",
        "sc_verified": "Verified",
        "sc_missing": "Missing",
        "sc_mismatch": "Mismatch",
        "sc_unmanifested": "Not in manifest",
        "sc_unchecked": "Hash not checked",
        "sc_invalid": "Invalid path",
        "sc_unsupported": "Unsupported algorithm",
        "sc_external": "External links",
        "sc_problems": "Problem files",
        "sc_paths_hidden": "File paths are hidden by private mode.",
        "col_state": "State", "col_path": "Path",
...
def build_metrics(tree, lang="ru", audit_result=None, ctx=None):
    ...
    for fid, fam in families.items():
        ...
        if mode == "share":
            # The historical subset only: omitted (possibly-living) children
            # must not even be counted — a count reveals their existence.
            kids = [ch for ch in kids
                    if ch.value and _included(ctx, tree.norm_id(ch.value))]
        n_kids = len(kids)
...
def _family_members(tree, fam):
    """All member ids of a family: spouses and children."""
    members = []
    for role in ("HUSB", "WIFE"):
        ref = fam.value_of(role)
        if ref:
...
def build_places(tree, ctx=None):
    ...
    for fam in tree.families.values():
        # A family "belongs" to its children too: a marriage place of a family
        # with a protected member (spouse OR child) is not disclosed.
        if any(not shown(m) for m in _family_members(tree, fam)):
            continue
...
def build_scan_check(tree, manifest_path, do_hash=False, include_paths=True):
    ...
    ``include_paths=False`` (private mode) keeps the counts but drops the
    per-file problem paths — private output strips machine paths everywhere
    else, so the integrity section must not reintroduce them.
    """
    ...
    return {
        ...
        "states": states,
        "problems": problems[:50] if include_paths
                    else [{"path": "", "state": p["state"]} for p in problems[:50]],
...
def build_timeline(tree, ctx=None):
    ...
    for fam in tree.families.values():
        ...
        # A marriage touching any protected/omitted member is suppressed —
        # including families with no recorded spouses but protected children
        # (their marriage year+place would otherwise still leak as "? & ?").
        if any(not shown(m) for m in _family_members(tree, fam)):
            continue
...
def render(metrics, template_path):
    with open(template_path, "r", encoding="utf-8") as fh:
        html = fh.read()
    # Substitution order matters: the library and title markers are consumed
    # FIRST, on the pristine template, so GEDCOM data that happens to contain a
    # marker string (e.g. "/*__CHARTJS__*/" in a note) stays inert JSON text
    # instead of being expanded.
    #
    # Inline the vendored Chart.js so the file needs no network at all. The
    # HTML parser matches "</script" case-insensitively, so the escape must
    # too; "<!--" inside script content can switch the parser into an escaped
    # state, so it is broken up as well (the vendored 4.4.3 build contains
    # neither sequence — this is a tripwire for future library updates).
    chartjs = _SCRIPT_CLOSE_RE.sub("<\\\\/script", _load_chartjs())
...
def main(argv):
    ...
    if manifest_path and mode == "share":
        print("note: --manifest is ignored in --share mode (a shared file "
              "must not expose machine paths)", file=sys.stderr)
    elif manifest_path:
        if not os.path.exists(manifest_path):
            print(f"manifest not found: {manifest_path}", file=sys.stderr)
            return 1
        try:
            metrics["scan_check"] = build_scan_check(
                tree, manifest_path, do_hash,
                include_paths=(mode == "none"))
